...
exports.idcheckio = function ( config ){
    ...
    if( !config || !config.user || !config.pwd ){
        ...
        self.user   = (config.user)?config.user:'NO_USER';
        self.pwd    = (config.pwd)?config.pwd:'NO_PWD';
        // Not mandatory other configs :
        self.language = (config.language == 'FR' || config.language == 'EN')?config.language:'EN';
        self.host     = (config.host)?config.host:prod.host;             // default to Prod
        self.protocol = (config.protocol)?config.protocol:prod.protocol; // default to Prod
        self.port     = (config.port)?config.port:prod.port;             // default to Prod
    }
    ...
    this.getReport = function( uid  , callback ) {
        clientR.get( '/rest/v0/pdfreport/'+uid , function (data, response) {
            if(err){ callback(err); }else{
                switch (response.statusCode) {
                    case 200 :
                        return callback(null, data);
